...
def test_export_chord_progressions_json(client, mock_controller):
    ...
    # Setup mock
    mock_controller.export_chord_progressions.return_value = json.dumps([
...
def test_export_chord_progressions_csv(client, mock_controller):
    ...
    # Setup mock
    mock_controller.export_chord_progressions.return_value = b"name,key,scale_type\nTest Progression 1,C,MAJOR\nTest Progression 2,G,MINOR"
...
def test_export_note_patterns(client, mock_controller):
    ...
    # Setup mock
    mock_controller.export_note_patterns.return_value = json.dumps([
        {"name": "Test Pattern 1"},
        {"name": "Test Pattern 2"}
    ])
...
def test_export_rhythm_patterns(client, mock_controller):
    ...
    # Setup mock
    mock_controller.export_rhythm_patterns.return_value = json.dumps([
        {"name": "Test Rhythm 1"},
        {"name": "Test Rhythm 2"}
    ])
...
def test_export_sequences(client, mock_controller):
    ...
    # Setup mock
    mock_controller.export_sequences.return_value = json.dumps([
...
def test_import_chord_progressions(client, mock_controller):
    ...
    with patch("note_gen.routers.import_export.get_import_export_controller", return_value=mock_controller):
        ...
        mock_controller.import_chord_progressions.assert_called_once()
...
def test_import_note_patterns(client, mock_controller):
    ...
    with patch("note_gen.routers.import_export.get_import_export_controller", return_value=mock_controller):
        ...
        assert response.status_code == 200
        assert response.json()["imported_count"] == 2
        mock_controller.import_note_patterns.assert_called_once()
...
def test_import_rhythm_patterns(client, mock_controller):
    ...
    with patch("note_gen.routers.import_export.get_import_export_controller", return_value=mock_controller):
        ...
        assert response.status_code == 200
        assert response.json()["imported_count"] == 2
        mock_controller.import_rhythm_patterns.assert_called_once()
...
def test_import_sequences(client, mock_controller):
    ...
    with patch("note_gen.routers.import_export.get_import_export_controller", return_value=mock_controller):
        ...
        assert response.status_code == 200
        assert response.json()["imported_count"] == 2
        mock_controller.import_sequences.assert_called_once()
