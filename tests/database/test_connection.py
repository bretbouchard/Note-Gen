...
from src.note_gen.database.connection import get_database_connection
...
async def test_get_database_connection_default_values():
    ...
    with patch('src.note_gen.database.connection.AsyncIOMotorClient') as mock_client:
        # Setup the mock
...
async def test_get_database_connection_custom_values():
    ...
    # Mock the AsyncIOMotorClient to avoid actual connection
    with patch('src.note_gen.database.connection.AsyncIOMotorClient') as mock_client, \
         patch('src.note_gen.database.connection.DATABASE', custom_db_config):
        # Setup the mock
        mock_instance = MagicMock()
        mock_client.return_value = mock_instance
        mock_db = MagicMock(spec=AsyncIOMotorDatabase)
