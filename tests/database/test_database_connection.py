...
from note_gen.database.db import init_db, get_db_conn, close_mongo_connection


@pytest_asyncio.fixture
async def mock_client():
    """Create a mock AsyncIOMotorClient."""
    with patch('note_gen.database.db._client', None):
        ...
        with patch('note_gen.database.db.AsyncIOMotorClient', return_value=client):
...
async def test_get_db_conn_with_existing_client():
    ...
    with patch('note_gen.database.db._client', mock_client), \
         patch('note_gen.database.db.settings.database_name', 'test_db'):
        # Call get_db_conn
...
async def test_get_db_conn_without_existing_client(mock_client):
    ...
    with patch('note_gen.database.db.settings.database_name', 'test_db'):
        # Call get_db_conn
        result = await get_db_conn()
