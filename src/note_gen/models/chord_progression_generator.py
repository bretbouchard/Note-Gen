...
from src.note_gen.models.note_sequence import NoteSequence  # Import NoteSequence
from src.note_gen.models.note import Note  # Update import statement
...
class ChordProgressionGenerator(BaseModel):
    ...
    @classmethod
    def validate_pattern(cls, v: Optional[ProgressionPattern]) -> Optional[ProgressionPattern]:
        if v is not None and not isinstance(v, ProgressionPattern):
            raise ValueError('pattern must be an instance of ProgressionPattern or None.')
        return v
    ...
    def generate(self, pattern: Optional[ProgressionPattern] = None, progression_length: int = 4) -> ChordProgression:
    ...
    def generate_custom(self, degrees: List[int], qualities: List[str]) -> ChordProgression:
        ...
        try:
            ...
            for degree, quality in zip(degrees, qualities):
                logger.info(f"Generating chord for degree: {degree}, quality: {quality}")
                ...
                if chord_notes is None or any(note is None or not isinstance(note, Note) for note in chord_notes):
                ...
                bass = Note.from_midi(root.midi_number - 12)  # Transpose down by one octave for bass
    ...
    def generate_chord(self, numeral: str) -> Chord:
        ...
        bass = Note.from_midi(root_note.midi_number - 12)  # Transpose down by one octave for bass
        logger.info(f"Creating bass note for degree: {scale_degree}, bass: {bass}")
        return Chord(root=root_note, quality=ChordQualityType(quality), notes=chord_notes, bass=bass)
