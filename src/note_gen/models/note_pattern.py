...
from typing import Any, Literal, Optional, Union, List, Dict, TYPE_CHECKING, Type
...
class NotePatternData(BaseModel):
    ...
    def validate_notes(cls, v: List[NoteType]) -> List[NoteType]:
        """Validate notes in the pattern. Notes cannot be empty."""
        if not v:
...
class NotePattern(BaseModel):
    ...
    def validate_name(cls: Type[NotePattern], v: Optional[str]) -> str:
        if not v:
