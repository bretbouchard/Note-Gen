...
from src.note_gen.models.chord_base import ChordBase, CHORD_INTERVALS, CHORD_COUNT
...
class Chord(MusicalBase, BaseModel):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        logging.debug(f"Initializing Chord with root: {root}, quality: {quality}, notes: {notes}")
        logging.debug(f"Root: {root.name} {root.octave}")
        logging.debug(f"Quality: {quality}")
        if notes is not None:
            logging.debug(f"Notes: {[note.name for note in notes]}")
        ...
        super().__init__(root=root, quality=quality, bass=bass, inversion=inversion, **kwargs)
